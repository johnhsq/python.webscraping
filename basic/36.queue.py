from urllib.request import urlopen
from bs4 import BeautifulSoup
import re
import random
import _thread
from queue import Queue
import time
import sqlite3

### DB operation
# $ sqlite3
# sqlite> attach database 'wiki.db' as wiki;
# sqlite> .database
# sqlite> .exit
# $ sqlite3 wiki.db
# sqlite> .database
# sqlite> CREATE TABLE pages (
#    id INTEGER PRIMARY KEY AUTOINCREMENT,
#    title TEXT,
#    path TEXT,
#    content TEXT,
#    created TIMESTAMP DEFAULT CURRENT_TIMESTAMP
# );
# sqlite> .tables
# sqlite> .schema pages
def storage(queue):
    db = '/Users/huang/Documents/Workspace/python.webscraping/basic/wiki.db'
    conn = sqlite3.connect(db)
    cur = conn.cursor()
    print("opened sqlite3 database successfully")
    while 1:
        if not queue.empty():
            article = queue.get()
            sql = 'SELECT * FROM pages WHERE path = "%s"' % (article["path"])
            print(sql)
            cur.execute(sql)
            rows=cur.fetchall()
            if len(rows) == 0:
                print("Storing article {}".format(article["title"]))
                cur.execute('INSERT INTO pages (title, path) VALUES ("%s", "%s")' % \
                    (article["title"], article["path"]))
                conn.commit()
            else:
                print("Article already exists: {}".format(article['title']))

visited = []
def getLinks(thread_name, bs):
    print('Getting links in {}'.format(thread_name))
    links = bs.find('div', {'id':'bodyContent'}).find_all('a',
        href=re.compile('^(/wiki/)((?!:).)*$'))
    return [link for link in links if link not in visited]

def scrape_article(thread_name, path, queue):
    visited.append(path)
    html = urlopen('http://en.wikipedia.org{}'.format(path))
    time.sleep(5)
    bs = BeautifulSoup(html, 'html.parser')
    title = bs.find('h1').get_text()
    print('Added {} for storage in thread {}'.format(title, thread_name))
    queue.put({"title":title, "path":path})
    links = getLinks(thread_name, bs)
    if len(links) > 0:
        newArticle = links[random.randint(0, len(links)-1)].attrs['href']
        scrape_article(thread_name, newArticle, queue)

queue = Queue()
try:
    # creates three threads: two to scrape pages from Wikipedia in a random walk, 
    # and a third to store the collected data in a MySQL database
    # 
    _thread.start_new_thread(scrape_article, ('Thread 1',
        '/wiki/Kevin_Bacon', queue,))
    _thread.start_new_thread(scrape_article, ('Thread 2',
        '/wiki/Monty_Python', queue,))
    _thread.start_new_thread(storage, (queue,))
except:
    print ('Error: unable to start threads')

while 1:
    pass